...
/** Code is written for check out validation... */

// function brsm_validate_region_stock_at_checkout() {
...
// 		$stock        = get_post_meta( $target_id, $region_key, true );

// 		if ( $stock === '' || $stock === false ) continue;
// 		if ( (int) $stock < (int) $qty ) {
// 			wc_add_notice( sprintf(
// 				__( 'Sorry, not enough stock for "%s" in region (%s). Available: %d', 'woo-region-stock-manager' ),
// 				get_the_title( $target_id ),
// 				strtoupper( $shipping_country ),
// 				$stock
// 			), 'error' );
// 		}
// 	}
...
//     // Optionally, redirect or throw a WP_Error to force stop if needed
//     // if ($error) { wp_die( ... ); }
// }
// add_action( 'woocommerce_checkout_process', 'brsm_validate_region_stock_at_checkout' );
